...
ration_text_width = round(56/7, 2)
all_text_y = 20
primary_text_bg='#5E6772'
primary_text='vikbert'
primary_padding = 5 # X-Value of primary text
primary_text_width = 3 * primary_padding + int(len(primary_text) * ration_text_width) # X-value of primary background
...
def add_text_with_shadow(dwg, text, x, y):
    ...
    shapes.add(dwg.text(text, insert=(x + 1, y + 1), fill='#000', font_size=20, font_family='Helvetica', fill_opacity=0.3))
    shapes.add(dwg.text(text, insert=(x, y), fill='#FFFFFF', font_size=20, font_family='Helvetica'))
    dwg.add(shapes)
...
def generate(secondary_text):
    secondary_text_x = primary_text_width + 2 * primary_padding

    secondary_text_bg = '#2196F3'
    container_width = 58 + int(len(secondary_text+primary_text) * ration_text_width)
    container_height = 25
    ...
    dwg = add_text_with_shadow(dwg, primary_text, primary_padding, all_text_y)
    dwg = add_text_with_shadow(dwg, secondary_text, secondary_text_x, all_text_y)
    dwg.save()

generate('DesignPattern')
generate('Flutter')
generate('Coding')